...
namespace RevAssuranceApi.Controllers
{
    ...
    public class DocumentRetrievalController : ControllerBase
    {
        ...
        DocumentRetImplementation _DocumentRetImplementation;
        ...
        public DocumentRetrievalController(
                                        ...
                                        DocumentRetImplementation DocumentRetImplementation,
                                          ...
                                          )
        {
            ...
            _DocumentRetImplementation = DocumentRetImplementation;
        ...
        public async Task<IActionResult> GetAllSearchHistory(ParamLoadPage AnyAuth)
        {
            try
            {
                ...
                var _response = await rtn.LoadData("isp_GetDocRetrivalListHis", param, db);
        ...
        public async Task<IActionResult> Add(DocumentRetrievalDTO p)
        {
            try
            {
                ...
                int SaveServiceChg = 0, chgDetails = 0, numberOfprDocChgDetail = 0,
                     successAddChgDetails = 0, serviceChargeCount = 0, successServiceCharge = 0, SaveDocChgDetails = 0;
                ...
                if (rev > 0)
                {
                    ...
                    foreach (var i in p.ListoprDocChgDetail)
                    {
                        ...
                        SaveDocChgDetails = await _DocumentRetImplementation.SaveDocRetCharges(i, p.LoginUserId);

                        //chgDetails = await _ApplicationDbContext.SaveChanges(p.LoginUserId);
                        if (SaveDocChgDetails > 0)
        ...
        public async Task<IActionResult> Update(DocumentRetrievalDTO p)
        {
            try
            {
                ...
                //  var serviceRef = await _ComputeChargesImplementation.GenServiceRef(p.ServiceId);
                //  oprDocRetrieval.ReferenceNo = serviceRef.nReference;
                var get = await _repooprDocRetrieval.GetAsync(x => x.ItbId == p.oprDocRetrieval.ItbId);
                if (get != null)
                {
                    if (get.AcctNo == p.oprDocRetrieval.AcctNo && get.AcctName == p.oprDocRetrieval.AcctName
                        && get.AcctType == p.oprDocRetrieval.AcctType && get.ServiceId == p.oprDocRetrieval.ServiceId
                        && get.AcctSic == p.oprDocRetrieval.AcctSic && get.AcctStatus == p.oprDocRetrieval.AcctStatus
                        && get.RsmId == p.oprDocRetrieval.RsmId && get.ProcessingDeptId == p.oprDocRetrieval.ProcessingDeptId
                    )
                    {
                        ApiResponse.ResponseCode = -99;
                ...
                get.AcctName = p.oprDocRetrieval.AcctName;
                get.AcctNo = p.oprDocRetrieval.AcctNo;
                get.AcctType = p.oprDocRetrieval.AcctType;
                get.ServiceId = p.oprDocRetrieval.ServiceId;
                get.AcctSic = p.oprDocRetrieval.AcctSic;
                get.AcctStatus = p.oprDocRetrieval.AcctStatus;
                get.RsmId = p.oprDocRetrieval.RsmId;
                get.ProcessingDeptId = p.oprDocRetrieval.ProcessingDeptId;

                _repooprDocRetrieval.Update(get);
