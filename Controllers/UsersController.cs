    ...
    public class UsersController : ControllerBase
    {
        ...
        public async Task<IActionResult> Update(admUserProfileDTO p)
        {
            try
            {
                ...
                if (get != null)
                {
                    if (get.FullName == p.FullName && get.LoginId == p.LoginId.ToUpper() &&
                    get.StaffId == (p.StaffId != "" ? p.StaffId : null) &&
                    get.RoleId == p.RoleId && get.DeptId == p.DeptId && get.BranchNo == p.BranchNo &&
                    get.Status == p.Status && get.EmailAddress == p.EmailAddress &&
                    get.CreatedBy == p.CreatedBy && get.CanPrintStatement == p.CanPrintStatement &&
                    ...
                    get.CanReverse == p.CanReverse && get.RsmId == p.RsmId &&
                    get.loginType == p.loginType && get.UseCbsLimit == p.UseCbsLimit)
                    {
                    ...
                    get.StaffId = p.StaffId != "" ? p.StaffId : "";
                    ...
                    get.Password = Cryptors.EncryptNoKey(p.LoginId.ToUpper());
                    get.loginstatus = 0;
                    get.lockcount = 0;
                    get.logincount = 0;

                    get.UseCbsAuth = p.UseCbsAuth;
                    get.UseCbsLimit = p.UseCbsLimit;
                    ...
                    get.loginType = p.loginType;

                    _ApplicationDbContext.admUserProfile.Update(get);
        ...
        public async Task<IActionResult> Add(admUserProfileDTO p)
        {
            try
            {
                ...
                {
                    ApiResponse.ResponseCode = -99;
                    ApiResponse.ResponseMessage = "User with Login Id Already Exist!";
                    return BadRequest(ApiResponse);
                }
                var getStaffExist = await _repoadmUserProfile.GetAsync(x => x.StaffId == p.StaffId);
                if (getStaffExist != null)
                {
                    ApiResponse.ResponseCode = -99;
                    ApiResponse.ResponseMessage = "User with Staff Id Already Exist!";
                    return BadRequest(ApiResponse);
                }
                else
                {
                    ...
                    //get.UserId
                    get.FullName = p.FullName;
                    get.LoginId = p.LoginId != "" ? p.LoginId.ToUpper() : "";
                    get.StaffId = p.StaffId != "" ? p.StaffId : "";
                    get.IsFirstLogin = false;
                    get.IsSupervisor = p.IsSupervisor;
                    get.LoggedOn = false;
            ...
            try
            {
                ...
                foreach (var b in p.admUserProfileDTO)
                {
                    ...
                    if (get != null)
                    {
                        ...
                        get.Password = Cryptors.EncryptNoKey(b.LoginId.ToUpper());
        ...
        public async Task<IActionResult> AddUserLimit(UserLimitsParamDTO param)
        {
            try
            {
                ...
                if (userLimit != null)
                {
                    if (userLimit.CreditLimit == param.UserLimitDTO.CreditLimit && userLimit.CreditLimit == param.UserLimitDTO.CreditLimit
                    ...
                    _ApplicationDbContext.admUserLimits.Update(userLimit);
