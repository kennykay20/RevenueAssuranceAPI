    ...
    public class InstrumentController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAllStatus()
        {
            try
            {
                ...
                var sql = "select Distinct InstrumentStatus from OprInstrument";
        ...
        public async Task<IActionResult> Add(InstrumentDTO p)
        {
            try
            {
                ...
                if (p.ServiceId == 19)
                {
                    tmp = _AmmendReprintReasonImplementation.replacePrintingAPG(p.TemplateContent, OprInstrument);
                    tem.TemplateContent = tmp;
                }
                else if (p.ServiceId == 11)
                {
                    tmp = _AmmendReprintReasonImplementation.replacePrintingBidSecuirty(p.TemplateContent, OprInstrument);
                    tem.TemplateContent = tmp;
                }
                else if (p.ServiceId == 18)
                {
                    tmp = _AmmendReprintReasonImplementation.replacePrintingPerformanceBond(p.TemplateContent, OprInstrument);
                    tem.TemplateContent = tmp;
                }
                else if (p.ServiceId == 20)
                {
                    tmp = _AmmendReprintReasonImplementation.replacePrintingCustomBond(p.TemplateContent, OprInstrument);
                    tem.TemplateContent = tmp;
                }
                else if (p.ServiceId == 22)
                {
                    tmp = _AmmendReprintReasonImplementation.retentionplacePrintingRetententionBond(p.TemplateContent, OprInstrument);
                    tem.TemplateContent = tmp;
                }
                else
                {
        ...
        public async Task<IActionResult> Update(InstrumentDTO p)
        {
            ...
            try
            {
                ...
                //var get = await _repoOprInstrument.GetAsync(x => x.ItbId == itbId);

                var get = _ApplicationDbContext.OprInstrument.FirstOrDefault(x => x.ItbId == itbId);

                if (get != null)
                {
                    if (get.AcctType == p.OprInstrument.AcctType && get.AcctNo == p.OprInstrument.AcctNo
                        && get.CcyCode == p.OprInstrument.CcyCode && get.AcctName == p.OprInstrument.AcctName
                        && get.Tenor == p.OprInstrument.Tenor && get.Purpose == p.OprInstrument.Purpose
                        && get.AddressLine1 == p.OprInstrument.AddressLine1 && get.AddressLine2 == p.OprInstrument.AddressLine2
                        && get.AcctSic == p.OprInstrument.AcctSic && get.ProcessingDeptId == p.OprInstrument.ProcessingDeptId
                        && get.BranchNo == p.OprInstrument.BranchNo && get.Beneficiary == p.OprInstrument.Beneficiary
                        && get.ContractDate == p.OprInstrument.ContractDate)
                    {
                        ApiResponse.ResponseCode = -99;
                ...
                get.AcctType = p.OprInstrument.AcctType;
                get.AcctNo = p.OprInstrument.AcctNo;
                get.CcyCode = p.OprInstrument.CcyCode;
                get.AcctName = p.OprInstrument.AcctName;
                get.Tenor = p.OprInstrument.Tenor;
                get.Purpose = p.OprInstrument.Purpose;
                get.AddressLine1 = p.OprInstrument.AddressLine1;
                get.AddressLine2 = p.OprInstrument.AddressLine2;
                get.AcctSic = p.OprInstrument.AcctSic;
                get.ProcessingDeptId = p.OprInstrument.ProcessingDeptId;
                get.BranchNo = p.OprInstrument.BranchNo;
                get.Beneficiary = p.OprInstrument.Beneficiary;
                get.ContractDate = p.OprInstrument.ContractDate;
                ...
                if (OprInstrument != null)
                {
                    ...
                    _ApplicationDbContext.OprInstrument.Update(get);
                    ...
                    foreach (var b in p.ListoprServiceCharge)
                    {
                        ...
                        _ApplicationDbContext.oprServiceCharges.Update(serviceCharges);
                    ...
                    if (p.ListoprCollateral.Count > 0)
                    {
        ...
        public async Task<IActionResult> CalCulateCharge(InstrumentDTO p)
        {
            try
            {
                ...
                var getFirstTemId = p.ListoprServiceCharge.Where(c => c.TemplateId != null).FirstOrDefault();
                if (getFirstTemId != null)
                {
                    tempIdGen = getFirstTemId.TemplateId;
                }
                ...
                foreach (var i in p.ListoprServiceCharge)
                {
                    ...
                    if (OperationViewModel.TempTypeId == null)
                    {
                        OperationViewModel.TempTypeId = tempIdGen;
                    }
                ...
                if (getCon != null)
                {
                    ...
                    if (getCon.ContDrAcctNo != null && getCon.ContDrAcctNo.Contains(con))
                    {
                    ...
                    if (getCon.ContCrAcctNo != null && getCon.ContCrAcctNo.Contains(con))
                    {
            ...
            {
                ApiResponse.ResponseMessage = ex == null ? ex.InnerException.Message : ex.Message;
                ApiResponse.ResponseMessage = "Could not compute charges, try later or contact system admin";
                ApiResponse.ResponseCode = -99;
                return BadRequest(ApiResponse);
            }
        }
        ...
        public async Task<IActionResult> GetById(InstrumentDTO p)
        {
            try
            {
                ...
                var userNameVal = await _repoadmUserProfile.GetAsync(x => x.UserId == Convert.ToInt32(p.OprInstrument.UserId));
                AccountValParam.Username = userNameVal.LoginId;
