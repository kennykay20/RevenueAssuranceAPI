...
namespace RevAssuranceApi.Controllers
{
    ...
    public class BranchController : ControllerBase
    {
        ...
        public async Task<IActionResult> Update(admBankBranch p)
        {
            try
            {
                ...
                if(get != null)
                {
                    ...
                    if( get.BranchNo == p.BranchNo && get.BranchCode == p.BranchCode && get.BranchAddress == p.BranchAddress
                        && get.BranchName == p.BranchName && get.BranchPreffix == p.BranchPreffix && get.BranchSuffix == p.BranchSuffix
                        && get.Status == p.Status && get.BranchAddress == p.BranchAddress && get.IsHeadOffice == p.IsHeadOffice &&
                        get.TelePhone == p.TelePhone && get.AltTelephone == p.AltTelephone
                    )
                   {
                       ApiResponse.ResponseCode = -99;
                ...
                {
                    ApiResponse.sErrorText = "Error occured when you are updating, kindly contact system admin";
                    return BadRequest(_response); 
                }
        ...
        public async Task<IActionResult> Add([FromBody] admBankBranch p)
        {
            try
            {
                p.IsHeadOffice = p.IsHeadOffice == null ? false : true;
