    ...
    public class TransConfigController : ControllerBase
    {
        ...
        public async Task<IActionResult> Update(admTransactionConfiguration p)
        {
            try
            {
                ...
                if (get != null)
                {
                    if (get.ServiceId == p.ServiceId && get.Direction == p.Direction &&
                       get.CustomerAcctCrTC == p.CustomerAcctCrTC &&
                       get.CustomerAcctDrTC == p.CustomerAcctDrTC &&
                       get.DrNarration.Trim() == p.DrNarration.Trim() &&
                       get.CrNarration.Trim() == p.CrNarration.Trim() &&
                       get.GLAcctDrTC == p.GLAcctDrTC &&
                       get.GLAcctCrTC == p.GLAcctCrTC &&
                       get.DrChargeNarr.Trim() == p.DrChargeNarr.Trim() &&
                       get.CrChargeNarr.Trim() == p.CrChargeNarr.Trim() &&
        ...
        [HttpPost("GetAllTranCodeConfig")]
        public async Task<IActionResult> GetAllTranCodeConfig(ParamLoadPage AnyAuth)
        {
            try
            {
                var roleAssign = await _RoleAssignImplementation.GetRoleAssign(AnyAuth.MenuId, AnyAuth.RoleId);

                var rtn = new DapperDATAImplementation<admTranCodeConfig>();

                string script = "Select * from admTranCodeConfig";
