...
namespace RevAssuranceApi.Controllers
{
    ...
    public async Task<IActionResult> ApproveTrans(PostTransDTO p)
    {
        try
        {
            ...
            if (Success == 0)
            {
                ApiResponse.ResponseMessage = "None of the Transaction(s) was Approved!";
                return BadRequest(ApiResponse);
            }
    ...
    public async Task<IActionResult> RejectTrans(PostTransDTO p)
    {
        try
        {
            ...
            foreach (var b in p.ListGetApproveServiceDTO)
            {
                    ...
                    string scr = $"select * from oprServiceCharges where ServiceItbId = {b.ItbId} and ServiceId = {b.ServiceId}";
