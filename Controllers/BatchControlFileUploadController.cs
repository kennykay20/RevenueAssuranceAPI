    ...
    public class BatchControlFileUploadController : ControllerBase
    {
        ...
        public async Task<IActionResult> Add(BatchControlDTO p)
        {
            try
            {
                ...
                var getOne  = p.BatchParamsItemTempDTOs.FirstOrDefault();
                //var dr = p.ListBatchItemsTemp.Where(c => c.DrCr == "DR");
                var dr = p.BatchParamsItemTempDTOs.Where(c => c.DrCr == "DR");
                //var cr = p.ListBatchItemsTemp.Where(c => c.DrCr == "CR");
                var cr = p.BatchParamsItemTempDTOs.Where(c => c.DrCr == "CR");
                ...
                var getTolCr = cr.Sum(c=> c.Amount);
                 ...
                 batc.RecordCount = p.BatchParamsItemTempDTOs.Count;
                ...
                string valErrorCodes = string.Empty;
                 ...
                 foreach(var b in p.BatchParamsItemTempDTOs)
                 {
                    ...
                    if(!string.IsNullOrWhiteSpace(b.AcctType))
                    {
                        ...
                        {
                            if(getAccType != null && getAccType.AcctLenght!= null && b.AcctNo != null){
                    ...
                    if(string.IsNullOrWhiteSpace(b.CbsTC))
                    {
                           ...
                           {
                               if(b.AcctNo != null && b.AcctNo.Length == 10)
                               {
                           ...
                           else if(b.DrCr != null && b.DrCr.Trim().ToLower() == "cr")
                           {
                               if(b.AcctNo != null && b.AcctNo.Length == 10)
                               {
                    ...
                    BatchItemsTemp bt = new BatchItemsTemp();
                    valErrorCodes = _Formatter.RemoveLast(valErrorCodes);
                    
                    _LogManager.SaveLog("b.ValueDate " + b.ValueDate);
                    
                    //check if serial date
                    string checkDate = (!string.IsNullOrEmpty(b.ValueDate)) ? _Formatter.CheckIfSerialDate(b.ValueDate) : null;
                    
                    seqNo += 1;

                    bt.AcctNo = b.AcctNo;
                    bt.AcctType = b.AcctType;
                    if(checkDate != null)
                    {
                        bt.ValueDate = Convert.ToDateTime(checkDate);
                    }
                    
                    bt.CbsTC = b.CbsTC;
                    bt.Narration = b.Narration;
                    bt.CcyCode = b.CcyCode;
                    bt.Amount = b.Amount;
                    bt.DrCr = b.DrCr;
                    bt.ChargeCode = b.ChargeCode;
                    bt.ChargeAmount = b.ChargeAmount;
                    bt.TransactionDate = b.TransactionDate;
                    //bt.ChargeAmount = b.ChargeAmount;
                    bt.ProcessingDept = b.ProcessingDept;
                    bt.DeptId = b.DeptId;
                    bt.ChequeNo = b.ChequeNo;
                    bt.OriginatingBranchId = b.OriginatingBranchId;
                    bt.ChgNarration = b.ChgNarration;

                    bt.BatchSeqNo = seqNo;
                    bt.ServiceStatus = "Loaded";
                    bt.DateCreated = DateTime.Now;
                    bt.UserId = p.LoginUserId;
                    bt.BatchNo = batc.BatchNo;
                    bt.ServiceId = p.ServiceId;
                    bt.ValErrorIds = valErrorCodes;
                    ...
                    await  _repoBatchItemsTemp.AddAsync(bt);
               ...
               if(batchInsert> 0 && rev > 0)
               {
                    ...
                    ApiResponse.ResponseMessage=  "Your Upload has Successfully Saved Temporarily";
                   var res = new 
                   {
                       batch = batc,
        ...
        public async Task<IActionResult> UpdateBatchItem(BatchControlDTO p)
        {
            try
            {
                ...
                var get = await _repoBatchItemsTemp.GetAsync(x => x.ItbId == p.BatchItemsTemp.ItbId);
                if(get != null)
                {
                    if(get.Narration == p.BatchItemsTemp.Narration && get.AcctNo == p.BatchItemsTemp.AcctNo 
                        && get.AcctType == p.BatchItemsTemp.AcctType && get.CcyCode == p.BatchItemsTemp.CcyCode
                        && get.CbsTC == p.BatchItemsTemp.CbsTC && get.Amount == p.BatchItemsTemp.Amount
                        && get.ChargeAmount == p.BatchItemsTemp.ChargeAmount && get.ChargeCode == p.BatchItemsTemp.ChargeCode
                        && get.ReferenceNo == p.BatchItemsTemp.ReferenceNo && get.ChgNarration == p.BatchItemsTemp.ChgNarration
                        && get.DrCr == p.BatchItemsTemp.DrCr)
                        {
                            ApiResponse.ResponseCode = -99;
                ...
                batchItemChg.UserId = p.LoginUserId;
                    ...
                    get.Narration = p.BatchItemsTemp.Narration;
                    get.AcctNo = p.BatchItemsTemp.AcctNo; 
                    get.AcctType = p.BatchItemsTemp.AcctType;
                    get.CcyCode = p.BatchItemsTemp.CcyCode;
                    get.CbsTC = p.BatchItemsTemp.CbsTC; 
                    get.Amount = p.BatchItemsTemp.Amount;
                    get.ChargeAmount = p.BatchItemsTemp.ChargeAmount;
                    get.ChargeCode = p.BatchItemsTemp.ChargeCode;
                    get.ReferenceNo = p.BatchItemsTemp.ReferenceNo;
                    get.ChgNarration = p.BatchItemsTemp.ChgNarration;
                    get.DrCr = p.BatchItemsTemp.DrCr;
                    get.UserId = p.LoginUserId;

                  _repoBatchItemsTemp.Update(get);
                    rev = await _ApplicationDbContext.SaveChanges(p.LoginUserId);
        ...
        public async Task<IActionResult> ValidateUpload(BatchControlDTO p)
        {
            try
            {
                 ...
                 foreach(var b in p.UploadBulkFilesValidatorList)
                 {
                   ...
                   if( (!string.IsNullOrWhiteSpace(b.ChargeCode)  && string.IsNullOrWhiteSpace(b.ChargeAmount)) ||
                        (!string.IsNullOrWhiteSpace(b.ChargeAmount)  && string.IsNullOrWhiteSpace(b.ChargeCode)))
                    {
                        valErrorCodes = _Formatter.AddComma("1009");
        ...
        public async Task<IActionResult> ProcessFileUpload(BatchControlDTO p)
        {
            ...
            try
            {
               ...
               {
                    ApiResponse.ResponseCode =   0;
                    ApiResponse.ResponseMessage=  "Loaded Successfully!";
