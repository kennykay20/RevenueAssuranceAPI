...
namespace RevAssuranceApi.Controllers
{
    ...
    public class BatchControlController : ControllerBase
    {
        ...
        public async Task<IActionResult> Update(BatchControlDTO p)
        {
            ...
            {
                BatchControl batch = new BatchControl();
                var get = await _repoBatchControl.GetAsync(x => x.BatchNo == p.BatchControl.BatchNo);
                if(get != null)
                {
                    if(get.BatchNo == p.BatchControl.BatchNo && get.Status == p.BatchControl.Status
                        && get.CcyCode == p.BatchControl.CcyCode && get.DefaultNar == p.BatchControl.DefaultNar)
                    {
                        ApiResponse.ResponseCode = -99;
                ...
                get.BatchNo = p.BatchControl.BatchNo;
                get.Status = p.BatchControl.Status;
                get.CcyCode = p.BatchControl.CcyCode;
                get.DefaultNar = p.BatchControl.DefaultNar;
        ...
        public async Task<IActionResult> RemoveBatch(BatchControlDTO p)
        {
            ...
            try
            {
                ...
                foreach (var b in p.ListBatchControl)
                {
                    ...
                    {
                        sb.Append($"<li> Failed: Batch No: { b.BatchNo }  wasn't Removed</li>");
        ...
        [HttpPost("UndoRejectBatch")]
        public async Task<IActionResult> UndoRejectBatch(BatchControlDTO p)
        {
            ApiResponse apiResponse = new ApiResponse();
            ...
            try
            {
                ...
                foreach (var b in p.ListBatchControl)
                {
                    ...
                    if (rev > 0)
                    {
                        sb.Append($"<li> Success: Batch No: { b.BatchNo }  Rejected Successfully!</li>");

                    }
                    else
                    {
                        sb.Append($"<li> Failed: Batch No: { b.BatchNo }  was'nt Rejected</li>");
        ...
        public async Task<IActionResult> ProcessAllItem(BatchControlDTO p)
        {
            ...
            try
            {
                ...
                {
                    ApiResponse.ResponseCode = 0;
                    ApiResponse.ResponseMessage = "Processed Successfully and Forwarded for Approval!";
