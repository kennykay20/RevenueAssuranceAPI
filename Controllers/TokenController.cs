...
namespace RevAssuranceApi.Controllers
{
    ...
    public class TokenController : ControllerBase
    {
        ...
        [HttpPost("GetAll")]
        public async Task<IActionResult> GetAll(TokenDTO AnyAuth)
        {
            try
            {
                ...
                if (RoleAssign != null)
                {
                     AnyAuth.pnGlobalView = RoleAssign.IsGlobalSupervisor == true ? "Y" : "N";
                }
                ...
                var rtn = new DapperDATAImplementation<OprTokenDTO>();
                
                var _response = await rtn.LoadData("isp_GetTokenlist", param, db);
        ...
        public async Task<IActionResult> GetAllSearchHistory(OprTokenDTO AnyAuth)
        {
            try
            {
                DynamicParameters param = new DynamicParameters();
                ...
                {
                    param.Add("@psStatus", AnyAuth.AcctStatus);
                }
                ...
                if(!string.IsNullOrEmpty(AnyAuth.AcctName))
                {
                    param.Add("@AccountName", AnyAuth.AcctName);
                }
                ...
                if(!string.IsNullOrEmpty(AnyAuth.pnBranchNo))
                {
                    param.Add("@psBranchNo", Convert.ToInt32(AnyAuth.pnBranchNo));
                }
                ...
                var _response = await rtn.LoadData("Isp_GetOprTokenSearchHis", param, db);

                if(_response != null)
                {
                   var getCharge = await _ChargeImplementation.GetChargeDetails(Convert.ToInt32(AnyAuth.ServiceId));

                   var admService = await _repoadmService.GetAsync(c=> c.ServiceId == Convert.ToInt32(AnyAuth.ServiceId)) ;
      ...
      //Process SIngle
        [HttpPost("Process")]
        public async Task<IActionResult> Process(TokenDTO p)
        {
            try
            {
            ...
            var rev = -1; 
            var getRec = await  _repooToken.GetAsync(c=> c.ItbId == p.OprToken.ItbId);
            if(getRec != null){

                   getRec.ProcessingDeptId = p.DeptId;
              getRec.ServiceStatus = UnauthorizedStatus;          
            _repooToken.Update(getRec);
       ...
       [HttpPost("ProcessList")]
        public async Task<IActionResult> ProcessList(TokenDTO p)
        {
            try
            {
            ...
            string UnauthorizedStatus =  _configuration["Statuses:UnauthorizedStatus"];;
             ...
             var admCharge = await _repoadmCharge.GetAsync(c=> c.ServiceId == p.ServiceId);
                if (admCharge != null)
                {
                      ...
                      var cbs = new DapperDATAImplementation<OprToken>();

                    string script = $"select * from OprToken where itbid in({tranIds})";
        ...
        public async Task<IActionResult> Update(TokenDTO p)
        {
            try
            {
                   ...
                   var getToken = await _repooToken.GetAsync(c=> c.ItbId == itbId);

                   if(getToken != null)
                   {
                       getToken.AcctNo = p.OprToken.AcctNo;
                      _repooToken.Update(getToken);
        ...
        public async Task<IActionResult> CalCulateCharge(TokenDTO p)
        {
           ...
           [HttpPost("GetById")]
        public async Task<IActionResult> GetById(TokenDTO p)
        {
            try
            {
                var serviceChargeslist = new List<oprServiceCharges>();
                OprToken serviceDetails = new OprToken();
                
                 serviceDetails = await _repooToken.GetAsync(c=> c.ItbId ==  p.ItbId);
                if(serviceDetails != null)
                {
                    serviceChargeslist = await _ServiceChargeImplementation.GetServiceChargesByServIdAndItbId(serviceDetails.ServiceId, (int)serviceDetails.ItbId);

                    var allUsers = await _UsersImplementation.GetAllUser(serviceDetails.UserId, serviceDetails.RejectedBy, serviceDetails.DismissedBy);
                    
                    var chargeSetUp = new List<admCharge>();

                if(serviceChargeslist.Count() == 0)
                {
                     chargeSetUp = await  _ChargeImplementation.GetChargeDetails(serviceDetails.ServiceId);
                ...
                foreach(var b in chargeSetUp)
                   {
                    ...
                    AccountValParam.AcctNo = serviceDetails.AcctNo;
                    AccountValParam.AcctType = serviceDetails.AcctType == null ? null : serviceDetails.AcctType.Trim();
                    
                    AccountValParam.CrncyCode = serviceDetails.CcyCode;
                    AccountValParam.Username = "System";
                    
                    var valServiceAcct = await _AccountValidationImplementation.ValidateAccountCall(AccountValParam);
