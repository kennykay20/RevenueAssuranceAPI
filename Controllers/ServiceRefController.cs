...
namespace RevAssuranceApi.Controllers
{
    ...
    public class ServiceRefController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll(AAuth AnyAuth)
        {
            try
            {
                var rtn = new DapperDATAImplementation<admServiceReferenceDTO>();
	...
	*,
	...
	from admServiceReference a";
        ...
        public async Task<IActionResult> Update(admServiceReference p)
        {
            try
            {
                var get = await _ApplicationDbContext.admServiceReference.FirstOrDefaultAsync(c => c.ItbId == p.ItbId);
                if (get != null)
                {

                    if (get.ServiceCode == p.ServiceCode && get.BankAbreviation == p.BankAbreviation &&
                        get.Frequency == p.Frequency && get.ReferenceFormat == p.ReferenceFormat &&
                        get.Status == p.Status)
                    {
                    ...
                    get.BankAbreviation = p.BankAbreviation == null ? get.BankAbreviation : p.BankAbreviation;
                    //get.CountryCode = p.CountryCode == null ? get.CountryCode : p.CountryCode;
                    //get.ReferenceNo = p.ReferenceNo;
                    get.Frequency = p.Frequency;
                    get.ReferenceFormat = p.ReferenceFormat;
                    //get.refFormat = p.refFormat;
