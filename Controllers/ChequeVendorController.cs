...
namespace RevAssuranceApi.Controllers
{
    ...
    public class ChequeVendorController : ControllerBase
    {
        ...
        public async Task<IActionResult> Update(ChqVendorDTO p)
        {
            ...
            var hund = p.Hundred == "true" ? "5" : "";
            string concatNotify = "";
            string concatProductCode = "";

            if (api == "" && webServ == "")
            {
                concatNotify = $"{email.Replace(",", "")}{api}{webServ}";
            }
            else if (email == "" && webServ == "")
            {
                concatNotify = $"{email}{api.Replace(",", "")}{webServ}";
            }
            else if (webServ == "")
            {
                concatNotify = $"{email}{api.Replace(",", "")}{webServ}";
            ...
            {
                concatNotify = $"{email}{api}{webServ}";
            }


            if (fift == "" && hund == "")
            {
                concatProductCode = $"{twen.Replace(",", "")}{fift}{hund}";
            }
            else if (twen == "" && hund == "")
            {
                concatProductCode = $"{twen}{fift.Replace(",", "")}{hund}";
            }
            else if (hund == "")
            {
                concatProductCode = $"{twen}{fift.Replace(",", "")}{hund}";
            ...
            {
                concatProductCode = $"{twen}{fift}{hund}";
            }
            ...
            try
            {
                ...
                if (get != null)
                {
                    if(get.VendorId == p.VendorId && get.VendorName == p.VendorName && get.VenAcctNo == p.VenAcctNo && get.Status == p.Status
                    && get.VenEmail == p.VenEmail && get.VenAltEmail == p.VenAltEmail && get.VenAcctType == p.VenAcctType && get.SoapDefinition == p.SoapDefinition
                    && get.APIEndPoint == p.APIEndPoint && get.VenAcctCcyCode == p.VenAcctCcyCode && get.NotificationType == concatNotify
                    && get.ChqProductCodes == concatProductCode) 
                    {
                    ...
                    get.VenAcctCcyCode = p.VenAcctCcyCode;
                    //logic for notification type
                    get.NotificationType = concatNotify;
                    get.ChqProductCodes = concatProductCode;
