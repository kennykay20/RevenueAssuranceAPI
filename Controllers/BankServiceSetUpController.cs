    ...
    public class BankServiceSetUpController : ControllerBase
    {
        ...
        public async Task<IActionResult> Add(admBankServiceSetupDTO p)
        {
            try
            {
                ...
                string Pwd = Cryptors.EncryptNoKey(p.admBankServiceSetup.Password);
        ...
        public async Task<IActionResult> Update(admBankServiceSetupDTO p)
        {
            try
            {
                ...
                if (get != null)
                {
                    var decrpPass = p.admBankServiceSetup.Password != null ? Cryptors.EncryptNoKey(p.admBankServiceSetup.Password) : null;
                    if (get.WebServiceUrl == p.admBankServiceSetup.WebServiceUrl && get.ConnectionName == p.admBankServiceSetup.ConnectionName
                    && get.DataBaseName == p.admBankServiceSetup.DataBaseName && get.DatabasePort == p.admBankServiceSetup.DatabasePort
                    && get.Server == p.admBankServiceSetup.Server && get.UserName == p.admBankServiceSetup.UserName && get.Password == decrpPass
                    ...
                    else
                    {
                        ...
                        {
                            string Encript = Cryptors.EncryptNoKey(p.admBankServiceSetup.Password);
                ...
                _ApplicationDbContext.admBankServiceSetup.Update(get);
                int _response = await _ApplicationDbContext.SaveChanges(p.LoginUserId);
                if (_response > 0)
                {
