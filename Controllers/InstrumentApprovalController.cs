        ...
        public async Task<IActionResult> ApproveInstrumentTrans(InstrumentDTO p)
        {
            ...
            try
            {
                ...
                foreach (var a in p.ListOprInstrument)
                {
                    ...
                    string trimDates = TransDate.Trim();
                    string lftrim = TransDate.TrimStart();
                    string rgttrim = TransDate.TrimEnd();

                    param.Add("@pnServiceId", a.ServiceId);
                    param.Add("@pnServiceItbId", a.ItbId);
                    param.Add("@pdtTransnDate", trimDates);

                    var ser = JsonConvert.SerializeObject(param);
                ...
                {
                    ApiResponse.ResponseMessage = "None of the Transaction was Approved!";
                    ApiResponse.ResponseCode = -99;
                    return BadRequest(ApiResponse);
                }
            ...
            {
                var exM = ex;
                ApiResponse.ResponseMessage = "An error Occured, please contact the System Admin!";
                ApiResponse.ResponseCode = -99;
                return BadRequest(ApiResponse);
            }
