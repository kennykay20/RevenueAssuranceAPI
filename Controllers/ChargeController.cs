        ...
        public async Task<IActionResult> Update([FromBody] admCharge p)
        {
            try
            {
                var get = _ApplicationDbContext.admCharge.FirstOrDefault(c => c.ItbId == p.ItbId);
                 if(get != null)
                 {
                     if(get.Description == p.Description && get.ServiceId == p.ServiceId &&
                     get.RecipientBranch == p.RecipientBranch && get.ChargeType == p.ChargeType &&
                     get.DRCR == p.DRCR && get.ChargeBasis == p.ChargeBasis && 
                     get.Direction == p.Direction && get.CurrencyIso == p.CurrencyIso &&
                     get.ChargeValue == p.ChargeValue && get.ChargeAcctType == p.ChargeAcctType &&
                     get.ChargeAcctNo == p.ChargeAcctNo && get.MinimumChargeCurr == p.MinimumChargeCurr &&
                     get.MinimumCharge == p.MinimumCharge && get.MaximumChargeCurr == p.MaximumChargeCurr &&
                     get.MaximumCharge == p.MaximumCharge && get.ChargeFlatCurr == p.ChargeFlatCurr && 
                     get.ChargeFlat == p.ChargeFlat && get.Ammendment == p.Ammendment &&
                     get.RePrint == p.RePrint && get.TemplateId == p.TemplateId && get.SubjectToTax == p.SubjectToTax)
                     {
                     ...
                     get.ChargeFlatCurr = p.ChargeFlatCurr;
                     get.ChargeFlat = p.ChargeFlat;
        ...
        public async Task<IActionResult> UpdateConcession(admChargeExemption p)
        {
            try
            {
                var get = _ApplicationDbContext.admChargeExemptions.FirstOrDefault(c => c.ItbId == p.ItbId);
                 if(get != null)
                 {
                     if(get.ChargeType == p.ChargeType &&
                        get.ChargeBasis == p.ChargeBasis &&
                        get.ChargeValue == p.ChargeValue &&
                        get.CurrencyIso == p.CurrencyIso &&
