...
namespace RevAssuranceApi.Controllers
{
    ...
    public class AuthTransController : ControllerBase
    {
        ...
        {
            _logManager.SaveLog("call AuthPostTransaction");
            int success = 0;
            int failure = 0;
            var apiResponse = new ApiResponse();
            var SubtringClass = new List<SubtringClassDTO>();
            var substringVal = new SubtringClassDTO();
            ...
            _logManager.SaveLog("call tranIds " + tranIds);
            ...
            {
                _logManager.SaveLog("call inside tranIds is not null" + tranIds);
                try
                {
                    ...
                    _logManager.SaveLog("call transactions  " + transactions);
                    ...
                    foreach (var k in transactions)
                    {
                        ...
                        {
                            _logManager.SaveLog("call i.DrAcctNo " + i.DrAcctNo);
                            try
                            {
                                var checkTransExit = await _CBSTransImplementation.ChectTransRefExist(i.ItbId, i.TransReference);
                                _logManager.SaveLog("call checkTransExit " + checkTransExit);
                                ...
                                if (postResp != null)
                                {
                                    ...
                                    {
                                         _logManager.SaveLog("call postResp.nErrorCode == 0 = " + postResp.nErrorCode);
                                        ...
                                        int SaveA = await _ApplicationDbContext.SaveChanges(p.UserId);
                                         _logManager.SaveLog("call SaveA = " + SaveA);
                                        ...
                                        if(updatePrimary.ResponseCode == 0){
                                            ...
                                            for(int j = 0; j < tracerCountSucc; j++){
                                                substringVal.TransTracer = i.TransTracer;
                                                substringVal.TransReference = i.TransReference;
                                            }
                                            _logManager.SaveLog("call substringVal = " + substringVal.TransTracer);
                                            SubtringClass.Add(substringVal);
                                             _logManager.SaveLog("add into SubtringClass = " + SubtringClass);
                                            continue;
                                        }
                    ...
                    {
                        _logManager.SaveLog("tracerCountSucc == count true");
                        ...
                        foreach(var b in SubtringClass) 
                        {
                            ...
                            if(getDefault == null)
                            {
                                ...
                                var listCount = getAllDefault.Count();
                                sb.Append($"<li><b> Success: Trans Tracer: { b.TransTracer } was Successful!</b> Trans Count: { listCount } </li>");
                                apiResponse.ResponseCode = 0;
                                 _logManager.SaveLog("Successful: Trans Tracer " + b.TransTracer);
            ...
            else
            {
                ...
                apiResponse.ResponseMessage = "No Transaction(s) were Selected";
