...
namespace RevAssuranceApi.Helper
{
    ...
    {
        private static string key = "Rev2020GMD";
         public static string Encrypt(string strToEncrpyt)
        {
            try
            {
        ...
        ////////
         ...
         public string EncryptRevAss(string clearText)
        {
            string EncryptionKey = "MAKV2SPBNI99212";
            byte[] clearBytes = Encoding.Unicode.GetBytes(clearText);
            using (Aes encryptor = Aes.Create())
        ...
        public string DecryptRevAss(string cipherText)
        {
            string EncryptionKey = "MAKV2SPBNI99212";
            cipherText = cipherText.Replace(" ", "+");
            byte[] cipherBytes = Convert.FromBase64String(cipherText);
