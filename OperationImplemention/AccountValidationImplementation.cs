...
namespace RevAssuranceApi.OperationImplemention
{
    ...
    public class AccountValidationImplementation
    {
        ...
        public async Task<AccountValidationDTO> ValidateLastChqNo(AccountValParam AccountValParam)
        {
            AccountValidationDTO values = new AccountValidationDTO();
            try
            {

                var get = await _FunctionApiSetUpImplementation.GetConnectDetails("ValidateMaxChqNo");

                var _responseBankServ = await _repoadmBankServiceSetup.GetAsync(c => c.Itbid == get.ConnectionId);
                var url = _responseBankServ.WebServiceUrl.Trim() + "/api/Services/ValidateMaxChqNo";
                ...
                {
                    // oValidateAcctResponse.nCheqNo = "200";
                    if (values.nLastChqNo != null)
                    {
                        values.nLastChqNo = values.nLastChqNo;
                    }
                }
                return values;
            }
