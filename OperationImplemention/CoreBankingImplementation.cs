...
namespace RevAssuranceApi.OperationImplemention
{
    ...
    public class CoreBankingImplementation
    {
        ...
        public async Task<List<ValidateChqRangeRsp>> InsertChqRange(ChequeBookReqDTO p)
        {
            ...
            try
            {
                ...
                foreach (var cbs in p.ListOprChqBookRequest)
                {
                    ...
                    {
                        psAcctNo = cbs.AcctNo,
                        psAcctType = cbs.AcctType,
                        pnFromChqNo = cbs.StartNo,
                        pnToChqNo = cbs.EndNo,
                        psUserName = "Phoenix",
                        pnVendorId = cbs.VendorId,
                        pnChqProductId = cbs.ChqProductCode,
                        pnCountOrdered = cbs.Quantity,
                        pnChargeAssessed = "0",
                        pnrimno = cbs.RsmId,
                        ConnectionStringId = get.ConnectionId
                    };
